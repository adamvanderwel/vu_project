...
export const FinancialOverview: React.FC<FinancialOverviewProps> = ({
  ...
  return (
    <div className="bg-white rounded-lg p-6">
      <h3 className="text-xl font-semibold mb-6">Financieel Overzicht</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-gray-600">Omzet Vandaag</span>
          </div>
          ...
          <div className="flex items-center gap-1 mt-2">
            ...
            <span className={metrics.revenueTrend > 0 ? "text-green-500" : "text-red-500"}>
              {Math.abs(metrics.revenueTrend).toFixed(1)}% vs gisteren
            </span>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="text-sm text-gray-500 mt-2">
            Gebaseerd op actuele productie
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-gray-600">Netdiensten Opbrengst</span>
          </div>
          <div className="text-2xl font-semibold">€{metrics.gridServicesRevenue.toFixed(2)}</div>
          <div className="text-sm text-gray-500 mt-2">
            Van balancering & congestiemanagement
          </div>
        </motion.div>
      </div>
      ...
      {metrics.negativePriceMetrics.hours > 0 && (
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-orange-700 font-medium">Negatieve Prijzen Vandaag</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <div className="text-orange-800">
                <span className="font-medium">{metrics.negativePriceMetrics.hours}</span> uren met negatieve prijzen
              </div>
              <div className="text-orange-700 text-sm mt-1">
                Gemiddeld €{Math.abs(metrics.negativePriceMetrics.avgNegativePrice).toFixed(2)}/MWh negatief
              </div>
            </div>
            <div>
              <div className="text-green-800">
                <span className="font-medium">€{metrics.negativePriceMetrics.avoidedLoss.toFixed(2)}</span> verlies vermeden
              </div>
              <div className="text-green-700 text-sm mt-1">
                Door slim productiemanagement
              </div>
            </div>
          </div>
        </motion.div>
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-green-700 font-medium">Besparingen Productiemanagement</span>
        </div>
        <div className="text-green-800">
          €{metrics.potentialLosses.toFixed(2)} bespaard door vermeden negatieve prijzen
        </div>
      </motion.div>
    </div>
  );
